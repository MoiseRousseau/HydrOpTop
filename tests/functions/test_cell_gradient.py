...
from HydrOpTop.Functions import Cell_Gradient
from common import __add_inputs__
import pytest

class Test_Cell_Gradient:
  ...
  def test_value_source_sink(self):
    ...
    obj = Cell_Gradient(cell_ids=center_id, variable="LIQUID_HEAD", power=1) #the center cell id where the sink is
    obj.cell_id_start_at = 1
    __add_inputs__(obj, self.sim_ss)
    cf = obj.evaluate(None)
    print(f"Cell Gradient: {cf} (should be very close to 0.)")
    assert cf < 1e-6
  ...
  def test_value_uniform(self):
    ...
    cf = obj.evaluate(None)
    print(f"Cell Gradient: {cf} (should be very close to 0.01)")
  ...
  def test_value_restricted(self):
    ...
    obj = Cell_Gradient(cell_ids=center_id, variable="LIQUID_HEAD", power=1, restrict_domain=True)
    obj.cell_id_start_at = 1
    __add_inputs__(obj,self.sim_ss)
  ...
  @pytest.mark.skip(reason="Tested elsewhere, to remove")
  def test_derivative_dP(self):
    ...
    obj = Cell_Gradient(cell_ids=np.arange(1,41), variable="LIQUID_HEAD", power=1.)
    obj.cell_id_start_at = 1
    __add_inputs__(obj, self.sim_ss)
    #get analytic derivative
    d_obj = obj.d_objective("LIQUID_HEAD", None)
    ...
    #perform comparison
    np.testing.assert_allclose(d_obj, d_obj_fd)
    return
  ...
  def test_derivative_dP_exp_grid(self):
    ...
    obj = Cell_Gradient(cell_ids=pit_ids, variable="LIQUID_HEAD", power=2.) 
  ...
  def test_derivative_dP_restrict_domain(self):
    ...
    #set up Head_Gradient object
    pit_ids = self.sim_exp_grid.get_region_ids("pit")
    obj = Cell_Gradient(cell_ids=pit_ids, variable="LIQUID_HEAD", restrict_domain=True) 
    __add_inputs__(obj, self.sim_exp_grid)
    obj.set_p_to_cell_ids(np.arange(1,self.sim_exp_grid.n_cells+1))
    #get analytic derivative
    d_obj = obj.d_objective_dY(None)[0]
