...
from HydrOpTop.Functions import p_Weighted_Cell_Gradient
from common import __add_inputs__

class Test_p_Cell_Gradient:
  ...
  def test_value_uniform(self):
    ...
    obj = p_Weighted_Cell_Gradient(power=1)
  ...
  def test_derivative_dp_partial(self):
    ...
    obj = p_Weighted_Cell_Gradient(ids_to_consider=pit_ids, power=2., restrict_domain=True)
  ...
  def test_derivative_dp_partial_invert_weighting(self):
    ...
    """
    pit_ids = self.sim_exp_grid.get_region_ids("pit")
    obj = p_Weighted_Cell_Gradient(ids_to_consider=pit_ids, power=1., invert_weighting=True)
    self.common_test_derivative_dp_partial(obj)
    return
