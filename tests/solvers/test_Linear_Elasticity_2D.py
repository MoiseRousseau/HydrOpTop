...
class Test_Total_Derivative:
  ...
  def test_total_derivative(self):
    ...
    S_adjoint = common.compute_sensitivity_adjoint(sim, cf, "YOUNG_MODULUS", matprop, Sensitivity_Richards, p=np.ones(n,dtype='f8'))
    cell_ids_to_test = np.arange(20,30)
    S_fd = common.compute_sensitivity_finite_difference(sim, cf, "YOUNG_MODULUS", matprop, cell_ids_to_test=cell_ids_to_test, pertub=1e-2, p=np.ones(n,dtype='f8'))
