...
class Head_Gradient:
  ...
  def d_objective_dP(self,p):
    ...
    d_grad = -self.vec_con * (1-self.fraction[:,np.newaxis])# - \
                 #(self.volume[:,np.newaxis]*self.grad_correction)[self.connection_ids[:,1]-1]
  ...
  def __initialize__(self):
    ...
    self.sorted_connections_i = np.argsort(self.ids_i)
    self.sorted_connections_j = np.argsort(self.ids_j)
    #bc correction
